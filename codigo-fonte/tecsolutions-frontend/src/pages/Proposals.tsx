// src/pages/institutional/Proposals.tsx
// Lista de propostas conectada ao backend (sem mocks e sem alterar layout)
...
const Proposals: React.FC = () => {
  ...
  // Filtro local (mantém layout/comportamento)
  ...
  // Excluir proposta e recarregar lista
  const handleDelete = async (id: string) => {
    if (!window.confirm("Tem certeza que deseja excluir esta proposta?"))
      return;
    const ok = await deleteProposal(id);
    if (ok) {
      // Recarrega do backend para manter consistência
      try {
  ...
  if (loading) {
    return (
      <div className="w-full h-full flex items-center justify-center">
        <span className="text-sm opacity-70">Carregando propostas…</span>
      </div>
    );
  }
                  ...
                  return (
                    <tr key={proposal.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {new Date(proposal.createdAt as any).toLocaleDateString(
                          "pt-BR"
                        )}
                      </td>
