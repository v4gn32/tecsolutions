...
// Hardware Inventory
export const getHardwareInventory = (): HardwareInventory[] => {
  const data = localStorage.getItem(STORAGE_KEYS.HARDWARE_INVENTORY);
  return data ? JSON.parse(data) : [];
};

export const saveHardwareInventory = (hardware: HardwareInventory): void => {
...
export const deleteHardwareInventory = (id: string): void => {
  const inventory = getHardwareInventory().filter(h => h.id !== id);
  localStorage.setItem(STORAGE_KEYS.HARDWARE_INVENTORY, JSON.stringify(inventory));
};

export const getHardwareByClient = (clientId: string): HardwareInventory[] => {
  return getHardwareInventory().filter(h => h.clientId === clientId);
};

// Software Inventory
export const getSoftwareInventory = (): SoftwareInventory[] => {
  const data = localStorage.getItem(STORAGE_KEYS.SOFTWARE_INVENTORY);
  return data ? JSON.parse(data) : [];
};

export const saveSoftwareInventory = (software: SoftwareInventory): void => {
...
export const deleteSoftwareInventory = (id: string): void => {
  const inventory = getSoftwareInventory().filter(s => s.id !== id);
  localStorage.setItem(STORAGE_KEYS.SOFTWARE_INVENTORY, JSON.stringify(inventory));
};

export const getSoftwareByClient = (clientId: string): SoftwareInventory[] => {
